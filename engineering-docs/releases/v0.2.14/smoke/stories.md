...
| S05/S10 session execution and evidence; S20 session report | `runner.py launch --project PATH`, `resume RUN_DIRECTORY`, `report RUN_DIRECTORY`; explicit actor observations through `note` | Website VSCodium startup and file/IDE resume confirmed by owner on RC0; attempts 2/3 passed exit and container-removal checks. No agent continuation or full fresh-image/sample coverage claimed. Per-session `--network host|bridge` is available. |
