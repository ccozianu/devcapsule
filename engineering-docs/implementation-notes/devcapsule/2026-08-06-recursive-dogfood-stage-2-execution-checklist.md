...
`devcapsule-src/pyproject.toml` with `[project].name = "devcapsule"`.
...
```text
devcapsule-src/dist/devcapsule.pex
```

Verify it from the repository root:

```bash
devcapsule-src/dist/devcapsule.pex version --json
```
...
```bash
devcapsule-src/dist/devcapsule.pex project \
...
```bash
devcapsule-src/dist/devcapsule.pex images build \
  --type environment \
