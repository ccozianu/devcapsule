# Intake: `sample-projects`

Work other workstreams have delivered to `sample-projects` and that `sample-projects` has not yet
...
`sample-projects` dispositions each item, records the outcome and its reasoning in its own
