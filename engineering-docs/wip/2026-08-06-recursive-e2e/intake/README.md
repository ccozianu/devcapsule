# Intake: `recursive-e2e`

Work other workstreams have delivered to `recursive-e2e` and that `recursive-e2e` has not yet
...
`recursive-e2e` dispositions each item, records the outcome and its reasoning in its own
