...
| `contained-display` | 2026-08-19 | Own the capsule supervisor core (container entry process, supervised children, explicit session end, headless mode) and the capsule's own display environment, closing the host-session credential exposure — supervisor first, display as its first consumer | active 2026-09-12; resumed at the owner's direction for the display-transport stage; the contained display is implemented on the working branch with unit, type and e2e evidence, awaiting PR delivery, a published recipe-8 base, and the owner's ratification day | `contained-display/display-transport`; `contained-display/outbox` | [workstream status](engineering-docs/wip/2026-08-19-contained-display/CURRENT-STATUS.md) |
