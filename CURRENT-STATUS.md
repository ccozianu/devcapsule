...
| `contained-display` | 2026-08-19 | Own the capsule supervisor core (container entry process, supervised children, explicit session end, headless mode) and the capsule's own display environment, closing the host-session credential exposure — supervisor first, display as its first consumer | active 2026-09-12; the contained display is merged to main (PR #75) and is the owner's dogfood since 2026-09-13; release candidates on `release-0.2.12` (`v0.2.12-rc3` passes the new mainline-integration gate); awaiting the published recipe-8 base, the release flip of the default, and the final tag | `contained-display/display-transport`; `contained-display/outbox` | [workstream status](engineering-docs/wip/2026-08-19-contained-display/CURRENT-STATUS.md) |
...
| `user-docs` | 2026-09-12 | Deliver V1 adopter documentation from first useful session through resuming work | open; awaiting owner review of initial focus | `user-docs/`; no branch yet | [workstream status](engineering-docs/wip/2026-09-12-user-docs/CURRENT-STATUS.md) |
