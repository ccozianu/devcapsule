# Review The Claim Test Design After CI Flakiness

Delivered: 2026-09-22
From: `project-management`, on explicit owner direction.

The owner requests a bug and xfail, not a quick timing fix. The current
project-management readiness slice adds only the non-strict assertion xfail
marker, the bug below and its index/status records; runtime and test body are
unchanged. Do not duplicate that quarantine. Repair belongs to workflow-improvements.

Please acknowledge and schedule contract-first review of claim identity,
renewal and expiry tests as detailed below. No release-blocking classification
or implementation semantics change has been decided. The bug will reach main
through project-management's ordinary integration. Until then its full record
is included here for immediate action; canonical path:
`engineering-docs/bugs/devcapsule/2026-09-22-workflow-claim-test-flakiness.md`.
