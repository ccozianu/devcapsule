...
def test_init_unverified_resolves_past_the_matrix_with_a_gentle_warning(
    ...
    with patch.dict(os.environ, isolated_env(tmp_path), clear=False), sparse_matrix():
        ...
        assert "Not yet validated: antigravity-cli 1.1.24 on base v0.2.12-rc3." in refusal
        assert "Run it as an experiment with --unverified" in refusal

        assert cli.main([*need, "--unverified"]) == 0
        warning = capsys.readouterr().err
        assert "Running as an experiment. Not yet validated: antigravity-cli 1.1.24 on base v0.2.12-rc3." in warning
...
def test_config_need_offers_and_accepts_the_experiment_lever(tmp_path: Path, capsys) -> None:
    ...
    with patch.dict(os.environ, isolated_env(tmp_path), clear=False), sparse_matrix():
        ...
        lock = read_toml(project / ".devcapsule" / "devcapsule.linux-amd64.lock")
        assert lock["unverified-combinations"] == "antigravity-cli 1.1.24 on base v0.2.12-rc3"
